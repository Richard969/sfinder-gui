          ...
          <div className="space-y-2">
            <div className="flex gap-1 flex-wrap">
              <button onClick={() => setSpinFilter('all')}
                className={`px-2 py-0.5 text-[10px] rounded-md font-medium transition-colors ${spinFilter === 'all' ? 'bg-primary/15 text-primary' : 'bg-background text-muted-foreground hover:bg-secondary'}`}>
                All ({spinRows.length})
              </button>
              {Object.entries(spinCats).map(([cat, count]) => (
                <button key={cat} onClick={() => setSpinFilter(cat)}
                  className={`px-2 py-0.5 text-[10px] rounded-md font-medium transition-colors ${spinFilter === cat ? 'bg-primary/15 text-primary' : 'bg-background text-muted-foreground hover:bg-secondary'}`}>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <button onClick={() => setPathShowThumbs(false)}
                className={`px-2 py-0.5 text-[10px] rounded-md font-medium ${!pathShowThumbs ? 'bg-primary/15 text-primary' : 'bg-background text-muted-foreground hover:bg-secondary'}`}>
                Table
              </button>
