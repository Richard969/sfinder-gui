...
export default function OutputViewer({ output, command, coverLogic }: OutputViewerProps) {
  ...
  const FumenThumbnail = ({ fumen, mark, clears, holes, pieces }: {
    ...
    // crop to visible rows (bottom rows with content)
    let top = 22;
    for (let y = 22; y >= 0; y--) {
      let empty = true;
    ...
    const markCls = mark === 'O' ? 'text-green-400' : mark === 'X' ? 'text-red-400' : 'text-muted-foreground';
    const markLabel = mark === 'O' ? '✓' : mark === 'X' ? '✗' : '-';
    ...
    return (
      <div className="flex flex-col items-center gap-1 p-2 rounded-lg border border-border bg-card hover:bg-accent cursor-pointer transition-colors"
        ...
        <div className={`text-xs font-bold ${markCls}`}>{markLabel}</div>
    ...
    return (
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-2 max-h-[500px] overflow-y-auto p-1">
          {filtered.map((s, i) => (
            s.fumen ? (
              <FumenThumbnail key={i} fumen={s.fumen} mark={s.mark} clears={s.clear} holes={s.hole} pieces={s.piece} />
            ) : (
